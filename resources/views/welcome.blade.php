            ...
            axios.get('/api/puertos')
                .then(response => {
                    response.data.forEach(puerto => {
                        try {
                            const coords = puerto.coordenadas_poligono.split(',').map(coord => {
                                return parseSingleCoordinate(coord);
                            });
            ...
            axios.get('/api/zonas')
                .then(response => {
                    response.data.forEach(zona => {
                        try {
                            const topLeftCoord = parseSingleCoordinate(zona.esquina_superior_izq);
                            const polygonCoords = calculateZonePolygon(
                                topLeftCoord,
                                zona.filas,
                                zona.contenedores_por_fila,
                                zona.separacion_entre_filas || 0,
                                zona.inclinacion_grados || 0
                            );
